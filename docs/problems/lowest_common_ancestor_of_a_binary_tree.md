...
The lowest common ancestor of `p` and `q` is where their ancestries meet: the
root-to-`p` path and the root-to-`q` path share a prefix, and the deepest node
of that shared prefix is the LCA. With no value ordering to exploit in a
general binary tree, every solution below must search for the two targets;
they differ in how the meeting point is detected.

1. **Start literal.** Materialize both root-to-target paths with a
   backtracking DFS, then walk them in lockstep until they diverge. Two
   passes over the tree and two stored paths: see
   [Find Root-to-Node Paths](#find-root-to-node-paths).
2. **Spot the waste.** The paths are compared once and discarded, and the
   divergence can be detected during the search itself. One post-order
   traversal lets each subtree report which target it contains; the first
   node that hears back from both sides is the LCA. Same `O(n)` time, one
   pass, no path storage: see [Recursive DFS](#recursive-dfs).
3. **Remove the recursion.** A tree of `10^5` nodes can be deep enough to
   overflow the call stack. Recording each node's parent during a BFS makes
   ancestry walkable upward without recursion: collect `p`'s ancestors into a
   set, then climb from `q` until the first member of that set appears: see
   [Iterative with Parent Pointers](#iterative-with-parent-pointers).
...
The code is the walkthrough written down: the backtracking path builder run
twice, then the lockstep comparison.
...
The path-based approach searches the tree twice and stores routes it only
compares once. The repair comes from noticing what the comparison actually
detects: the LCA is the deepest node from which `p` and `q` are
...
node. The Invariant below states the call contract this rests on: each call
...
Let us run the single pass on Example 1: `root = [3,5,1,6,2,0,8,null,null,7,4]`,
`p = 5`, `q = 1`, the tree
...
The indentation shows the recursion; each call either matches a target and
returns itself, or waits for both children to report:
...
The left call matches `p` at once and returns node `5` without descending: by
the Invariant, `5` is the correct report for its whole subtree whether or not
`q` also hides beneath it. The right call likewise returns node `1`. Back at
the root, `left` and `right` are both non-`None`: one target was found on each
side, so `3` is the split point and is returned. That is the expected Output
`3`, found in one traversal with no path lists.

#### Solution

The code is the walkthrough's contract in four lines: match, recurse both
sides, return the split point or propagate the single find.
...
The recursive solution can overflow the call stack on a deeply skewed tree of
`10^5` nodes. Instead of recursing, this approach makes the tree's implicit upward edges
...
Let us run the three phases on Example 2, where the BFS must dig several levels
before both targets are mapped: `root = [3,5,1,6,2,0,8,null,null,7,4]`, `p = 5`,
`q = 4`, the tree
...
Phase one builds the `parent` map level by level. The map starts as
`{3: None}` and the `queue` as `[3]`; the loop keeps going while either target
...
The BFS stops with `0` and `8` still in the queue: once both targets have
parents, nothing deeper matters. Phase two climbs from `p = 5` collecting
`ancestors`: add `5`, step to `parent[5] = 3`, add `3`, and `parent[3] = None`
ends the climb with `ancestors = {5, 3}`. Phase three climbs from `q = 4`
testing membership at each stop:
...
The first node on `q`'s upward path that also lies on `p`'s path is `5`, so
node `5` is returned: `p` is its own ancestor here, matching the expected
Output `5`.

#### Solution

The code is the walkthrough's three phases in order: BFS until both targets
are mapped, collect `ancestors` of `p`, climb from `q`.

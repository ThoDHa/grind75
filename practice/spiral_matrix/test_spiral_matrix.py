"""Tests for Spiral Matrix — your attempt (solution.py) against both case sets.

cases.json (marker `simple`) is the "Run" set; cases_full.json (marker `full`) is
the "Submit" gauntlet. The worked approaches live in ../../docs/problems/spiral_matrix.md.
"""
...
def test_solution_simple(case):
    try:
        _check(solution.Solution().spiralOrder, case)
    except NotSolved:

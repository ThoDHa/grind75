"""Tests for Partition Equal Subset Sum — your attempt (solution.py) against both case sets.

cases.json (marker `simple`) is the "Run" set; cases_full.json (marker `full`) is
the "Submit" gauntlet. The worked approaches live in
../../docs/problems/partition_equal_subset_sum.md.
"""
...
def test_solution_simple(case):
    try:
        _check(solution.Solution().canPartition, case)
    except NotSolved:

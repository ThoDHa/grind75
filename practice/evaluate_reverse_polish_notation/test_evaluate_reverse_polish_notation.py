"""Tests for Evaluate Reverse Polish Notation — your attempt (solution.py) against both case sets.

cases.json (marker `simple`) is the "Run" set; cases_full.json (marker `full`) is
the "Submit" gauntlet. The worked approaches live in
../../docs/problems/evaluate_reverse_polish_notation.md.
"""
...
def test_solution_simple(case):
    try:
        _check(solution.Solution().evalRPN, case)
    except NotSolved:

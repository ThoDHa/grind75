"""Tests for Word Search — your attempt (solution.py) against both case sets.

cases.json (marker `simple`) is the "Run" set; cases_full.json (marker `full`) is
the "Submit" gauntlet. The worked approaches live in ../../docs/problems/word_search.md.
"""
...
def _check(method, case):
    # exist mutates the board in place, so hand it a fresh copy per call.
    board, word = case["args"]
    board_copy = [row[:] for row in board]
    assert method(board_copy, word) == case["expected"]
...
def test_solution_simple(case):
    try:
        _check(solution.Solution().exist, case)
    except NotSolved:

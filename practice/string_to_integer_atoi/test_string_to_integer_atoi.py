"""Tests for String to Integer (atoi) — your attempt (solution.py) against both case sets.

cases.json (marker `simple`) is the "Run" set; cases_full.json (marker `full`) is
the "Submit" gauntlet. The worked approaches live in
../../docs/problems/string_to_integer_atoi.md.
"""
...
def test_solution_simple(case):
    try:
        _check(solution.Solution().myAtoi, case)
    except NotSolved:
